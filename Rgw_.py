...
#            result_rate = np.append(result_rate,10**4*kde([5.01*i+0.01,5.01*j+0.01,60])/dR_det(5.01*i+0.1,5.01*j+0.1,0.6))
...
#   plt.show()
    ...
    yuyutech = alfa_z(30.0,30.0,0.5)

    print yuyutech

    alfa_result = np.append(alfa_result,yuyutech)

print "mean\n", np.mean(alfa_result)
print "covariance\n", np.cov(alfa_result, rowvar=False)

plt.hist(alfa_result, bins=100)
...
#       ax = plt.subplot(111)
#       im = ax.imshow(ha.reshape(20,20),cmap="inferno")

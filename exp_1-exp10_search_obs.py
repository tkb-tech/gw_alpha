...
for tkb in range(0,i_a):
    for dq7 in range(0,i_b):
        ...
        def R_th( m_1 ,m_2 ,z):
            return Const_in_R_th * t_c(z)**(-34.0/37.0) *( m_1 + m_2 )**( _alpha_ ) * f( m_1 ) * f( m_2 ) * ( ( m_1 * m_2 ) / ( m_1 + m_2 )**2)**( _beta_ )
        ...
        print cnt
        probability_multinomial.append([_alpha_,_beta_,baki,p_value,p_total])
        uc = np.append(uc,p_value)



ax = plt.subplot(111)
im = ax.imshow(uc.reshape((i_a,i_b)))
